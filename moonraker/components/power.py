...
class TradfriDevice(PowerDevice):
    def __init__(self, config: ConfigHelper) -> None:
        super().__init__(config)
        ...
        if (len(self.address) == 0 or len(self.psk) == 0 or len(self.identity) == 0 or len(self.device_name) == 0):
            self.server.add_warning(
                f"Power Device {self.name}: Missing Address, Security Key or Device Name"
            )

    async def _ensure_api_factory(self) -> None:
        if self.api_factory is None:
            self.api_factory = await APIFactory.init(host=self.address, psk_id=self.identity, psk=self.psk)

    async def init_state(self) -> None:
        await self._ensure_api_factory()
        await self._find_tradfri_device()

    async def _find_tradfri_device(self) -> None:
        await self._ensure_api_factory()
        if self.api_factory is not None:
            ...
            for dev in devices:
                if dev.name == self.device_name:
                    self.device = dev
                    new_state = "on" if dev.socket_control.sockets[0].state else "off"
    ...
    async def set_power(self, state) -> None:
        await self._ensure_api_factory()
        await self._find_tradfri_device()
        if self.api_factory is not None and self.device is not None:
            api = self.api_factory.request
            switch_command = self.device.socket_control.set_state(True if state == "on" else False)
